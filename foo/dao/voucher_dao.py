...
class voucher_dao(singleton):
    __voucher_collection = None;

    def __init__(self):
        if self.__voucher_collection is None:
            conn = pymongo.MongoClient(MONGO_HOST, MONGO_PORT);
            db = conn[MONGO_DB];
            db.authenticate(MONGO_USR, MONGO_PWD);
            self.__voucher_collection = db.voucher;
        else:
            logging.info("voucher_dao has inited......");


    def create(self, json):
        self.__voucher_collection.insert(json);
        logging.info("create voucher success......");


    def update_status(self, _id, status):
        self.__voucher_collection.update({"_id":_id}, {"$set":{"status":status}});
        logging.info("update voucher status success......");


    def update(self, json):
        _id = json["_id"];
        self.__voucher_collection.update({"_id":_id},{"$set":json});
        logging.info("update voucher success......");

    # 已改为只返回有效的代金券
    def query_pagination_by_vendor(self, vendor_id, account_id, status, before, limit):
        cursor = self.__voucher_collection.find({
                "vendor_id":vendor_id,
                "account_id":account_id,
                "status":status,
                "expired_time":{"$gt":before}}).sort("create_time",-1).limit(limit);
        array = []
        for i in cursor:
            array.append(i)
        return array
    ...
    def query_not_safe(self, _id):
        cursor = self.__voucher_collection.find({"_id":_id})
        data = None
        for i in cursor:
            data = i
        return data
